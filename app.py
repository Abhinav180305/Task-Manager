...
# runs once at startup, before anything below reads os.environ, so values
# from .env behave exactly like variables you'd set with "export".
load_dotenv()

app = Flask(__name__)

# Flask needs a secret key to sign session cookies (that's what keeps a
# logged-in user logged in between requests). Set FLASK_SECRET_KEY in your
# .env file (or real environment) for actual use; the fallback below is
# fine for quick local testing only — don't ship it as-is.
app.secret_key = os.environ.get("FLASK_SECRET_KEY", "dev-only-change-me")

# ---------------------------------------------------------------------------
# MySQL connection settings
# All of these can be set in a .env file (see .env.example) or as real
# environment variables, so each person running this doesn't need to edit
# the code to match their own setup:
#
#   MYSQL_HOST      (default: localhost)
#   MYSQL_PORT      (default: 3306, MySQL's standard port)
#   MYSQL_USER      (default: root)
#   MYSQL_PASSWORD  (default: "" — set this to your real password)
#   MYSQL_DATABASE  (default: taskmanager)
# ---------------------------------------------------------------------------
DB_CONFIG = {
    ...
    "database": os.environ.get("MYSQL_DATABASE", "taskmanager"),
}
